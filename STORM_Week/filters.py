...
def filter_switcher(data, settings):
    switcher = {
        'kernel' : kernel_filter(data, settings["input parameter a"]),
        'DOG' : diff_of_gauss(data, settings("input parameter a"), settings("input parameter b")),
    }

    return switcher.get(settings["filter_type:"], data)
